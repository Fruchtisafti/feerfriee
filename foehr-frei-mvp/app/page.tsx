...
function fmtHuman(date:Date){ return new Intl.DateTimeFormat('de-DE',{day:'2-digit',month:'short'}).format(date); }
function rangeDays(start:Date,endExclusive:Date){ const res:Date[]=[]; let d=new Date(start); while(d<endExclusive){ res.push(new Date(d)); d.setDate(d.getDate()+1);} return res; }
function normalizeICS(icsText:string){ return icsText.replace(/\r?\n[ \t]/g,''); }
function parseICS(icsText:string){
  const text=normalizeICS(icsText); const events:{start:Date;end:Date}[]=[];
  const parts=text.split(/BEGIN:VEVENT/).slice(1).map(b=>'BEGIN:VEVENT'+b);
  for(const block of parts){
    const a=block.match(/DTSTART[^:]*:([^\r\n]+)/); const b=block.match(/DTEND[^:]*:([^\r\n]+)/);
    if(!a||!b) continue; const start=parseICSDate(a[1]); const end=parseICSDate(b[1]);
...
function computeWindows(booked:Set<string>, horizonDays=30){
  ...
  const windows:{start:Date;length:number}[]=[]; let cur:null|{start:Date;length:number}=null;
  for(const d of days){ const ymd=fmtYMD(d), free=!booked.has(ymd);
    if(free){ if(!cur) cur={start:new Date(d),length:1}; else cur.length+=1; }
    else if(cur){ windows.push(cur); cur=null; }
  }
  if(cur) windows.push(cur); return windows;
}
function nextWindowMeetingNights(w:{start:Date;length:number}[], nights:number, fromDate=todayAtMidnight()){
  for(const win of w){ if(win.start>=stripTime(fromDate) && win.length>=nights) return win; } return null;
}
...
function CalendarPreview({bookedSet,horizonDays=30}:{bookedSet:Set<string>|null;horizonDays?:number}){
  ...
  return (
    <div className="grid grid-cols-7 gap-1 text-xs">
      {days.map((d)=>{ const ymd=fmtYMD(d); const free=!bookedSet?.has(ymd);
        return (
...
type Listing = {
  id:string; title:string; location:string; persons:number; priceFrom:number;
...
function GuestSearch({listings,onLoadDemo}:{listings:Listing[];onLoadDemo:()=>void}){
  ...
  const results=useMemo(()=>{ const fromDate=new Date(start);
    return listings.map(l=>{ const matchVillage=village==='Alle Orte'||l.location===village;
    ...
    }).filter(x=>x.ok).sort((a,b)=> (a.window!.start as any) - (b.window!.start as any));
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-1 sticky top-4 h-max p-4 border rounded-lg bg-white">
        <div className="mb-3"><label className="block text-sm font-medium">Startdatum</label>
          <input type="date" value={start} onChange={(e)=>setStart(e.target.value)} className="mt-1 w-full border rounded px-2 py-1" /></div>
        <div className="mb-3"><label className="block text-sm font-medium">Nächte</label>
          <input type="number" min={1} max={30} value={nights} onChange={(e)=>setNights(Number(e.target.value))} className="mt-1 w-full border rounded px-2 py-1" /></div>
        <div className="mb-3"><label className="block text-sm font-medium">Personen</label>
          <input type="number" min={1} max={10} value={persons} onChange={(e)=>setPersons(Number(e.target.value))} className="mt-1 w-full border rounded px-2 py-1" /></div>
        <div className="mb-3"><label className="block text-sm font-medium">Preis bis (ab)</label>
          ...
          <p className="text-xs text-slate-500 mt-1">0 = egal</p></div>
        <div className="mb-3"><label className="block text-sm font-medium">Ort</label>
          <select value={village} onChange={(e)=>setVillage(e.target.value)} className="mt-1 w-full border rounded px-2 py-1">
            <option>Alle Orte</option>{VILLAGES.map(v=><option key={v}>{v}</option>)}
          </select></div>
      </div>
        ...
        {results.map(({listing,window})=>(
          <div key={listing.id} className="border rounded-lg bg-white">
            <div className="p-4 flex items-center justify-between border-b">
              ...
              <a href={listing.bookingUrl} target="_blank" rel="noreferrer" className="px-4 py-2 rounded bg-slate-900 text-white">Jetzt anfragen</a>
            </div>
...
function HostOnboarding({ listings, addListing, setListings }:{
  ...
  const [icsText,setIcsText]=useState<string>(''); const [error,setError]=useState<string>('');
  const [bookedSet,setBookedSet]=useState<Set<string>|null>(null); const [windows,setWindows]=useState<{start:Date;length:number}[]>([]);
  const [loadingICS,setLoadingICS]=useState<boolean>(false);

  function resetPreview(){ setBookedSet(null); setWindows([]); setError(''); }

  async function fetchICS(){
    setError(''); setLoadingICS(true);
    try{ const res=await fetch(icsUrl); const text=await res.text();
      setIcsText(text); const events=parseICS(text); const b=buildBookedSet(events); setBookedSet(b); setWindows(computeWindows(b));
    } catch(e){ setError('Konnte ICS nicht laden (CORS/URL prüfen). Nutze alternativ das Textfeld unten.'); }
    finally{ setLoadingICS(false); }
  }

  function parseFromTextarea(){
    setError('');
    try{ const events=parseICS(icsText); const b=buildBookedSet(events); setBookedSet(b); setWindows(computeWindows(b)); }
    catch(e){ setError('Konnte ICS nicht parsen. Prüfe das Format (DTSTART/DTEND).'); }
  }

  function handleSave(){
    if(!title || !bookingUrl || (!icsUrl && !icsText)){ setError('Titel, Buchungslink und Kalender sind erforderlich.'); return; }
    const id = safeUUID();
    const payload:Listing={ id, title, location, persons, priceFrom, bookingUrl, icsUrl: icsUrl || undefined, bookedSet: bookedSet!, windows };
    addListing(payload);
    setTitle(''); setPersons(2); setPriceFrom(0); setBookingUrl(''); setIcsUrl(''); setIcsText(''); setBookedSet(null); setWindows([]);
  }

  function clearAll(){
    if(confirm('Alle gespeicherten Unterkünfte wirklich löschen?')){ setListings([]); localStorage.removeItem('foehrfrei_listings'); }
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-2 space-y-4">
        <div className="border rounded-lg bg-white">
          <div className="p-4 border-b"><div className="text-lg font-semibold">Unterkunft hinzufügen</div></div>
          <div className="p-4 space-y-4">
            {error && <div className="p-3 text-sm border border-red-300 bg-red-50 rounded">{error}</div>}
            <div className="grid md:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium">Titel</label>
                <input className="mt-1 w-full border rounded px-2 py-1" placeholder="z. B. Haus MOIN – OG-Whg" value={title} onChange={(e)=>setTitle(e.target.value)} /></div>
              <div><label className="block text-sm font-medium">Ort</label>
                <select className="mt-1 w-full border rounded px-2 py-1" value={location} onChange={(e)=>setLocation(e.target.value)}>{VILLAGES.map(v=><option key={v}>{v}</option>)}</select></div>
              <div><label className="block text-sm font-medium">Max. Personen</label>
                <input type="number" min={1} max={12} className="mt-1 w-full border rounded px-2 py-1" value={persons} onChange={(e)=>setPersons(Number(e.target.value))} /></div>
              <div><label className="block text-sm font-medium">Preis ab (€/Nacht)</label>
                <input type="number" min={0} className="mt-1 w-full border rounded px-2 py-1" value={priceFrom} onChange={(e)=>setPriceFrom(Number(e.target.value))} /></div>
              <div className="md:col-span-2"><label className="block text-sm font-medium">Buchungs-/Kontaktlink</label>
                <input className="mt-1 w-full border rounded px-2 py-1" placeholder="https://… (eigene Website, Airbnb, Booking, WhatsApp)" value={bookingUrl} onChange={(e)=>setBookingUrl(e.target.value)} /></div>
            </div>
      ...
      <div className="space-y-4">
        <div className="border rounded-lg bg-white">
          <div className="p-4 border-b"><div className="text-lg font-semibold">Gespeicherte Unterkünfte</div></div>
          <div className="p-4 space-y-3">
            {listings.length===0 && <p className="text-sm text-slate-500">Noch nichts gespeichert. Demo-Daten laden oder erste Unterkunft hinzufügen.</p>}
            {listings.map(l=>(
              <div key={l.id} className="flex items-center justify-between p-2 border rounded-lg">
                <div><div className="font-medium">{l.title}</div><div className="text-xs text-slate-500">{l.location} · ab {l.priceFrom || '—'} €</div></div>
                <button className="px-3 py-2 rounded border" onClick={()=>{ if(confirm('Unterkunft löschen?')){ const next=listings.filter(x=>x.id!==l.id);
                  localStorage.setItem('foehrfrei_listings', JSON.stringify(next.map(n=>({...n,bookedSet:Array.from(n.bookedSet)})))); location.reload(); }}}>Löschen</button>
              </div>
            ))}
            {listings.length>0 && (
              <button className="px-3 py-2 rounded border" onClick={()=>{ if(confirm('Alle gespeicherten Unterkünfte wirklich löschen?')){ localStorage.removeItem('foehrfrei_listings'); location.reload(); }}}>Alle löschen</button>
            )}
          </div>
        </div>
...
function makeDemoICS(bookedRanges:[number,number][]){
  ...
  const start0=todayAtMidnight(); const ymd=(d:Date)=>`${d.getFullYear()}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}`;
  for(const [a,b] of bookedRanges){ const start=addDays(start0,a), end=addDays(start0,b); lines.push('BEGIN:VEVENT'); lines.push(`DTSTART;VALUE=DATE:${ymd(start)}`); lines.push(`DTEND;VALUE=DATE:${ymd(end)}`); lines.push('SUMMARY:Gebucht'); lines.push('END:VEVENT'); }
  lines.push('END:VCALENDAR'); return lines.join('\r\n');
}

function loadDemo(setListings:(l:Listing[])=>void){
  const demo1ICS=makeDemoICS([[2,5],[10,12],[18,22]]); const demo2ICS=makeDemoICS([[1,2],[6,9],[15,16],[25,27]]);
  const l1Events=parseICS(demo1ICS), l1Booked=buildBookedSet(l1Events), l1Windows=computeWindows(l1Booked);
  const l2Events=parseICS(demo2ICS), l2Booked=buildBookedSet(l2Events), l2Windows=computeWindows(l2Booked);
  const demoListings:Listing[]=[
    { id:safeUUID(), title:'Strandnahes Apartment – Südstrand', location:'Wyk', persons:3, priceFrom:89, bookingUrl:'https://example.com/wyk-apartment', bookedSet:l1Booked, windows:l1Windows },
    { id:safeUUID(), title:'Reetdach-Häuschen – Ruhig & gemütlich', location:'Nieblum', persons:4, priceFrom:129, bookingUrl:'https://example.com/nieblum-haus', bookedSet:l2Booked, windows:l2Windows }
  ];
  setListings(demoListings); const serial=demoListings.map(l=>({...l, bookedSet:Array.from(l.bookedSet)})); localStorage.setItem('foehrfrei_listings', JSON.stringify(serial));
}
